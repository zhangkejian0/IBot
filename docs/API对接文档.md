...
**视觉物体感知（XBot 端侧已发送，建议服务端纳入 `[感知 ...]`）：**

XBot 通过端侧 YOLO + 手部检测，会在 `perception` 中额外携带以下字段，用于回答「我手里拿的是什么 / 画面里有什么、在哪儿」。**当前服务端若未消费这些字段，`[感知 ...]` 中将无物体信息，大模型会凭空臆造（如把「手机」答成「书」），务必接入。**

```json
{
  "objects": ["手机", "水杯"],
  "objects_detail": [
    { "name": "手机", "confidence": 0.59, "location": "画面右下方", "box": [0.62, 0.55, 0.88, 0.91] },
    { "name": "水杯", "confidence": 0.41, "location": "画面左上方", "box": [0.05, 0.10, 0.22, 0.38] }
  ],
  "held_object": "手机",
  "held_object_detail": { "name": "手机", "confidence": 0.59, "location": "画面右下方", "box": [0.62, 0.55, 0.88, 0.91] },
  "scene": "克俭手里拿着手机（位于画面右下方，识别置信度59%）；画面中还能看到：水杯（画面左上方，识别置信度41%）"
}
```

| 字段 | 说明 |
|------|------|
| `objects` | 去重后的物体名列表（简名），如 `["手机","水杯"]` |
| `objects_detail` | 物体明细数组，每项含 `name`、`confidence`(0..1)、`location`(中文方位)、`box`(归一化 `[left,top,right,bottom]`，已按镜像校正与 `location` 一致) |
| `held_object` | 当前被手持物体名（手部+物体框空间叠加判定），用于回答「我手里拿的是什么」 |
| `held_object_detail` | 手持物体明细，结构同 `objects_detail` 单项 |
| `scene` | 端侧拼装的自然语言场景（含手持物/可见物的位置与置信度），最利于大模型直接理解；建议优先纳入 `[感知 ...]` |

> 建议服务端至少把 `held_object`（或 `scene`）拼进 `[感知 ...]`，否则视觉提问无法正确回答。
