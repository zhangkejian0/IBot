...
{
    private static final String TAG = "flutter_bt_serial";
    ...
    public void connect(String address, UUID uuid) throws IOException {
        ...
        // 方式1：标准 SDP 发现（createRfcommSocketToServiceRecord）
        try {
            ...
            Log.w(TAG, "SDP 连接失败，尝试反射 channel fallback: " + e.getMessage());
            lastError = e;
            if (socket != null) {
        ...
        // 方式2（fallback）：反射调用 createRfcommSocket(int channel)，直接连 channel 1
        // 部分国产 ROM（OPPO/小米等）SDP 服务发现不稳定，直连 channel 最可靠。
        if (socket == null) {
            for (int channel = 1; channel <= 5 && socket == null; channel++) {
                try {
                    ...
                    Log.w(TAG, "channel " + channel + " 连接失败: " + e.getMessage());
                    lastError = e;
                    if (socket != null) {
                        try { socket.close(); } catch (Exception ignored) {}
                        socket = null;
