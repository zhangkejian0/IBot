...
import { Face } from './face/render/Face';
import { NeonFace } from './face/render/NeonFace';
...
const FaceComponent =
  styleParam === 'neon' ? NeonFace
  : styleParam === 'ambient' ? AmbientFace
  : Face;
...
export function App() {
  ...
  const toggleDebug = () => {
    if (debugAvailable) closeDebug();
    else openDebug();
  };

  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === 'Escape' && debugAvailable && panelOpen) {
        e.preventDefault();
        hidePanel();
        return;
      }
      if (e.key === '`' || e.key === '~') {
        if (debugAvailable) setPanelOpen((v) => !v);
      }
  ...
  return (
    <div style={styles.root}>
      ...
      {debugAvailable && panelOpen && (
        <div style={styles.panel}>
          <div style={styles.tabBar}>
            <button
              style={tab === 'states' ? styles.tabActive : styles.tab}
      ...
      {debugAvailable && !panelOpen && (
        <button type="button" style={styles.reopenBtn} onClick={() => setPanelOpen(true)}>
          调试
        </button>
      )}
    </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  panel: {
    ...
    background: '#15171f', borderLeft: '1px solid #232636',
    display: 'flex', flexDirection: 'column', overflow: 'hidden',
    fontSize: 13, color: '#cfd2dc',
    zIndex: 900,
  },
  tabBar: {
    display: 'flex', alignItems: 'center', padding: '8px 8px',
    borderBottom: '1px solid #232636', gap: 6,
  },
  tab: {
    background: 'transparent', color: '#8a8f9f', border: 'none',
    padding: '8px 12px', borderRadius: 6, cursor: 'pointer', fontSize: 13,
    touchAction: 'manipulation',
  },
  tabActive: {
    background: '#2a2e44', color: '#fff', border: 'none',
    padding: '8px 12px', borderRadius: 6, cursor: 'pointer', fontSize: 13,
    touchAction: 'manipulation',
  },
  hideBtn: {
    ...
    background: 'transparent', color: '#8a8f9f',
    border: '1px solid #232636', cursor: 'pointer', fontSize: 18, lineHeight: 1,
    touchAction: 'manipulation',
  },
  closeBtn: {
    minWidth: 52, height: 36, padding: '0 10px', borderRadius: 6,
    background: '#3a2030', color: '#ffb8cc',
    border: '1px solid #5a3040', cursor: 'pointer', fontSize: 13,
    touchAction: 'manipulation',
  },
  panelBody: { flex: 1, overflowY: 'auto', padding: 12, WebkitOverflowScrolling: 'touch' as any },
  reopenBtn: {
    position: 'fixed', right: 12, bottom: 12, zIndex: 800,
    padding: '10px 14px', borderRadius: 8,
    background: 'rgba(21, 23, 31, 0.88)', color: '#cfd2dc',
    border: '1px solid #232636', cursor: 'pointer', fontSize: 13,
    touchAction: 'manipulation',
  },
};
